...
int main(int argc, char** argv) {
	...
	pcl::io::loadPLYFile("D:/code/PCD/自建配准点云/scene+rt/c1.ply", *cloud_source);
	...
	pcl::io::loadPLYFile("D:/code/PCD/自建配准点云/scene+rt/c2.ply", *cloud_target);
	...
	*key_source = *key_detect(cloud_source, normals_source, tree_source, 5.0f * leaf_size);
	*features_source = *com_features(cloud_source, normals_source, tree_source, key_source, 10.0f * leaf_size);
	...
	*key_target = *key_detect(cloud_target, normals_target, tree_target, 5.0f * leaf_size);
	*features_target = *com_features(cloud_target, normals_target, tree_target, key_target, 10.0f * leaf_size);
